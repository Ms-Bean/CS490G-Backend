...
async function get_client_profile_info(user_id)
{
    ...
    let get_client_profile_sql = "SELECT about_me, experience_level, height, weight, medical_conditions, budget, goals, target_weight, birthday FROM User_Profile WHERE user_id = ?"
...
async function set_client_profile_info(user_id, about_me, experience_level, height, weight, medical_conditions, budget, goals, target_weight, birthday)
{
    let set_client_profile_sql = "UPDATE User_Profile SET about_me=?, experience_level=?, height=?, weight=?, medical_conditions=?, budget=?, goals=?, target_weight=?, birthday=? WHERE user_id=?"
    console.log(set_client_profile_sql);
    console.log([about_me, experience_level, height, weight, medical_conditions, budget, goals, target_weight, birthday, user_id]);
    return new Promise((resolve, reject) =>{
        con.query(set_client_profile_sql, [about_me, experience_level, height, weight, medical_conditions, budget, goals, target_weight, birthday, user_id], function(err, results){
            if(err)
            {
                console.log(err);
                reject("sql failure");
